/**
 * Agent persistence — filesystem (JSONL) and in-memory implementations.
 */
...
export class FileSystemAgentRecordPersistence implements AgentRecordPersistence {
  ...
  constructor(
    ...
    private readonly options: { onError?: (error: unknown) => void } = {},
  ) {}
  ...
  async *read(): AsyncIterable<AgentRecord> {
    ...
    let line = "";
    let lineNumber = 0;
    const stream = createReadStream(this.filePath, { encoding: "utf8" as const });
    try {
