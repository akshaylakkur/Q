 ...
 *
 * Tools discovered from MCP servers are prefixed with `mcp__` and separated
 * by `__` to avoid collisions with built-in tools. If the qualified name
 * would exceed 64 characters, a stable 8-character FNV-1a hash suffix
 * replaces the tail.
 */
 ...
 * any run of `_` into a single underscore. The collapse step guarantees neither
 * the sanitized server nor tool name contains the `__` separator used by
 * {@link qualifyMcpToolName}, which lets decoders split unambiguously on the
