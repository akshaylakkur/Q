/**
 * Revision Input Component — Text input for capturing revision instructions
 * in the modus-maximus confirmation flow.
 *
...
export class RevisionInputComponent extends Container implements Focusable {
  focused = false;
  ...
  render(width: number): string[] {
    ...
    // ── Box top ───────────────────────────────────────────────────────
    const topBorder = chalk.hex(this.colors.info)("┌─ Revision ─────────────────────────────────────┐");
    ...
    // ── Input field ───────────────────────────────────────────────────
    const borderChar = chalk.hex(this.colors.info)("│");
    const inputField = chalk.hex(this.colors.primary)("> ");
    const displayText = this.value;

    // Cursor marker — we embed the CURSOR_MARKER for hardware cursor positioning
    ...
    const hint = chalk.hex(this.colors.textDim)(
      "  [Enter submit] [Esc cancel] [← → navigate]",
    );
