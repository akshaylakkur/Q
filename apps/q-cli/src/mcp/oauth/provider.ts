 ...
 *    state under `$HOME/.Q/credentials/mcp/<key>-*.json` (mode 0o600).
 *  - Captures the authorization URL when the SDK calls
 *    `redirectToAuthorization` — the McpOAuthService reads that field
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("q-cli (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `q-cli (${options.serverName})`;
  }
