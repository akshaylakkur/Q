 ...
 *
 * The service owns one McpOAuthClientProvider per server/resource and
 ...
 *     as `UnauthorizedError` from the transport instead of being swallowed.
 ...
 *     that finishes the code exchange once the user finishes the browser flow.
 *  3. After complete() resolves successfully the provider has tokens on disk;
 *     the caller (the synthetic tool) drives a manager-level reconnect to
 *     swap the synthetic tool out for the real MCP tools.
 */
...
export interface McpOAuthServiceOptions {
  /** Storage backend; overrides `homeDir` when supplied. */
...
export class McpOAuthService {
  ...
  constructor(options: McpOAuthServiceOptions = {}) {
    this.store = options.store ?? new JsonFileStore();
